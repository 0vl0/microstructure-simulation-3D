...
loc_lines = np.load(fileManager._get_path_3D_lines_image('1578_100_1e-05_50_3_4'))
#loc_lines[:,:,:,0] = 0
#loc_lines[:,:,:,1] = 0
print(np.unique(loc_lines))
#X, Y, Z = np.where(loc_lines[:,:,:,3]<70)
X, Y, Z = np.where(loc_lines[:,:,:,3]==0)
pos = np.array(list(zip(X,Y,Z)))
N = loc_lines[X,Y,Z].shape[0]
#colors = ColorArray(loc_lines[X,Y,Z].astype(np.float64)/255.0)
colors = ColorArray(loc_lines[X,Y,Z].astype(np.float64))
print('colors = ', colors)
print('colors.mean() = ', np.max(loc_lines[X,Y,Z]))
p_lines = Scatter3D(parent=view.scene)
p_lines.set_gl_state('opaque', blend=False, depth_test=True)
p_lines.set_data(pos, face_color=colors, symbol='o', size=1,
            edge_width=0, edge_color=[0,0,0,0])

micro = np.load(fileManager._get_path_npy_image('1578_100_1e-05_50_3_4'))
...
#axes = Axes(parent=view.scene, width=10)
